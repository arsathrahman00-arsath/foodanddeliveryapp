...
import { CalendarIcon, Package, Loader2, Trash2, Save, X, ArrowLeft } from "lucide-react";
...
const NewMaterialReceiptPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (successCount > 0 && duplicates.length === 0 && otherErrors.length === 0) {
        navigate("/dashboard/material-receipt");
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate("/dashboard/material-receipt")}>
          <ArrowLeft className="w-5 h-5" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold text-foreground">New Material Receipt</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select purchase request details, add categories, and record received quantities</CardDescription>
        </CardHeader>
...
export default NewMaterialReceiptPage;
